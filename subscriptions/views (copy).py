...
class SubscribeList(abstract.TemplateView):
  ...
  #  context_object_name = 'subscription'
    ...
    def get(self, request, *args, **kwargs):
        ...
        plan = models.SubscriptionPlan.objects.filter(active=True).first()
        #subscription = models.SubscriptionPlan.objects.filter(post_count=post_count).order_by('order')
        # Retrieve the plan details for template display
        details = models.PlanListDetail.objects.filter(
            plan_list=plan_list, plan=plan,plan__costs__isnull=False,
        ).order_by('order')

        if plan_list and plan:
            response = TemplateResponse(
                request,
                self.template_name,
                self.get_context_data(plan_list=plan_list, details=details, plan=plan,)
            )
