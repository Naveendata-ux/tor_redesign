...
class SubscribeList(abstract.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
       ...
       # plan_list = models.SubscriptionPlan.objects.filter(active=True).first()
        ...
        if details:
            response = TemplateResponse(
                request,
                self.template_name,
                self.get_context_data( details=details)
            )
...
class SubscribeView(LoginRequiredMixin, abstract.TemplateView):
    ...
    template_preview = 'subscriptions/checkout.html'
    ...
    def post(self, requesst, *args, **kwargs):  # pylint: disable=unused-argument
